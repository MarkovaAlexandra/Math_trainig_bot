...
import random
from Keyboard import create_kb_dif10
...
@dp.callback_query_handler(callback.filter(btn='next_dif10'))
@dp.callback_query_handler(callback.filter(menu='dif_up10'))
async def multipl(message: Message | CallbackQuery):
    ...
    await bot.edit_message_media(media=InputMediaPhoto(media=settings.PICTURE, caption=f'{a} - {b} = ?'),
                                 chat_id=chat_id,
                                 message_id=message_id, reply_markup=create_kb_dif10(settings.ENTER))

@dp.callback_query_handler(callback.filter(menu='main_dif_10'))
async def enter(call: CallbackQuery):
     ...
     await bot.edit_message_media(media=InputMediaPhoto(media=settings.PICTURE, caption=f'твой ответ {int(res)}?'),
                                  chat_id=chat_id,
                                  message_id=message_id, reply_markup=create_kb_dif10(int(res)))
